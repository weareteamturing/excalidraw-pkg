import "./chunk-7AOJXSRG.js";

// locales/pt-PT.json
var labels = {
  paste: "Colar",
  pasteAsPlaintext: "Colar como texto simples",
  pasteCharts: "Colar gr\xE1ficos",
  selectAll: "Selecionar tudo",
  multiSelect: "Adicionar elemento \xE0 sele\xE7\xE3o",
  moveCanvas: "Mover \xE1rea de desenho",
  cut: "Cortar",
  copy: "Copiar",
  copyAsPng: "Copiar para a \xE1rea de transfer\xEAncia como PNG",
  copyAsSvg: "Copiar para a \xE1rea de transfer\xEAncia como SVG",
  copyText: "Copiar para \xE1rea de transfer\xEAncia como texto",
  copySource: "Copiar origem para \xE1rea de transfer\xEAncia",
  convertToCode: "Converter para c\xF3digo",
  bringForward: "Subir para a frente de tudo",
  sendToBack: "Baixar",
  bringToFront: "Subir",
  sendBackward: "Baixar para tr\xE1s de tudo",
  delete: "Apagar",
  copyStyles: "Copiar estilos",
  pasteStyles: "Colar estilos",
  stroke: "Contorno",
  changeStroke: "Alterar cor do tra\xE7o",
  background: "Fundo",
  changeBackground: "Alterar cor do fundo",
  fill: "Preenchimento",
  strokeWidth: "Espessura do tra\xE7o",
  strokeStyle: "Estilo do tra\xE7o",
  strokeStyle_solid: "S\xF3lido",
  strokeStyle_dashed: "Tracejado",
  strokeStyle_dotted: "Pontilhado",
  sloppiness: "Precis\xE3o do tra\xE7o",
  opacity: "Opacidade",
  textAlign: "Alinhamento do texto",
  edges: "Arestas",
  sharp: "Reto",
  round: "Arredondado",
  arrowheads: "Pontas",
  arrowhead_none: "Nenhuma",
  arrowhead_arrow: "Seta",
  arrowhead_bar: "Barra",
  arrowhead_circle: "C\xEDrculo",
  arrowhead_circle_outline: "C\xEDrculo (contorno)",
  arrowhead_triangle: "Tri\xE2ngulo",
  arrowhead_triangle_outline: "Tri\xE2ngulo (contorno)",
  arrowhead_diamond: "Losango",
  arrowhead_diamond_outline: "Losango (contorno)",
  arrowhead_crowfoot_many: "P\xE9 de coroa (muitos)",
  arrowhead_crowfoot_one: "P\xE9 de coroa (um)",
  arrowhead_crowfoot_one_or_many: "P\xE9 de coroa (um ou muitos)",
  more_options: "Mais op\xE7\xF5es",
  arrowtypes: "Tipo de seta",
  arrowtype_sharp: "Seta retil\xEDnea",
  arrowtype_round: "Seta curva",
  arrowtype_elbowed: "Seta em cotovelo",
  fontSize: "Tamanho do tipo de letra",
  fontFamily: "Fam\xEDlia do tipo de letra",
  addWatermark: 'Adicionar "Feito com Excalidraw"',
  handDrawn: "Manuscrito",
  normal: "Normal",
  code: "C\xF3digo",
  small: "Pequeno",
  medium: "M\xE9dio",
  large: "Grande",
  veryLarge: "Muito grande",
  solid: "S\xF3lido",
  hachure: "Sombreado",
  zigzag: "Ziguezague",
  crossHatch: "Sombreado trama",
  thin: "Fino",
  bold: "Negrito",
  left: "Esquerda",
  center: "Centro",
  right: "Direita",
  extraBold: "Extra negrito",
  architect: "Arquiteto",
  artist: "Artista",
  cartoonist: "Caricaturista",
  fileTitle: "Nome do ficheiro",
  colorPicker: "Seletor de cores",
  canvasColors: "Usado na \xE1rea de desenho",
  canvasBackground: "Fundo da \xE1rea de desenho",
  drawingCanvas: "\xC1rea de desenho",
  clearCanvas: "Apagar \xE1rea de desenho",
  layers: "Camadas",
  actions: "A\xE7\xF5es",
  language: "Idioma",
  liveCollaboration: "Colabora\xE7\xE3o ao vivo...",
  duplicateSelection: "Duplicar",
  untitled: "Sem t\xEDtulo",
  name: "Nome",
  yourName: "O seu nome",
  madeWithExcalidraw: "Feito com Excalidraw",
  group: "Agrupar sele\xE7\xE3o",
  ungroup: "Desagrupar sele\xE7\xE3o",
  collaborators: "Colaboradores",
  toggleGrid: "Ativar/desativar grelha",
  addToLibrary: "Adicionar \xE0 biblioteca",
  removeFromLibrary: "Remover da biblioteca",
  libraryLoadingMessage: "A carregar a biblioteca\u2026",
  libraries: "Procurar bibliotecas",
  loadingScene: "A carregar a cena\u2026",
  loadScene: "Carregar cena de um ficheiro",
  align: "Alinhamento",
  alignTop: "Alinhar ao topo",
  alignBottom: "Alinhar ao fundo",
  alignLeft: "Alinhar \xE0 esquerda",
  alignRight: "Alinhar \xE0 direita",
  centerVertically: "Centrar verticalmente",
  centerHorizontally: "Centrar horizontalmente",
  distributeHorizontally: "Distribuir horizontalmente",
  distributeVertically: "Distribuir verticalmente",
  flipHorizontal: "Inverter horizontalmente",
  flipVertical: "Inverter verticalmente",
  viewMode: "Modo de visualiza\xE7\xE3o",
  share: "Partilhar",
  showStroke: "Mostrar seletor de cores do tra\xE7o",
  showBackground: "Mostrar seletor de cores do fundo",
  showFonts: "Mostrar seletor de fonte",
  toggleTheme: "Alternar tema claro/escuro",
  theme: "Tema",
  personalLib: "Biblioteca pessoal",
  excalidrawLib: "Biblioteca do Excalidraw",
  decreaseFontSize: "Reduzir o tamanho do tipo de letra",
  increaseFontSize: "Aumentar o tamanho do tipo de letra",
  unbindText: "Desligar texto",
  bindText: "Ligar texto ao recipiente",
  createContainerFromText: "Envolver texto num recipiente",
  link: {
    edit: "Editar hiperliga\xE7\xE3o",
    editEmbed: "Editar hiperliga\xE7\xE3o incorpor\xE1vel",
    create: "Adicionar hiperliga\xE7\xE3o",
    label: "Hiperliga\xE7\xE3o",
    labelEmbed: "Hiperligar e incorporar",
    empty: "N\xE3o est\xE1 definida nenhuma hiperliga\xE7\xE3o",
    hint: "Escreva ou cole a sua hiperliga\xE7\xE3o aqui",
    goToElement: "Ir para o elemento alvo"
  },
  lineEditor: {
    edit: "Editar linha",
    editArrow: "Editar seta"
  },
  polygon: {
    breakPolygon: "",
    convertToPolygon: ""
  },
  elementLock: {
    lock: "Bloquear",
    unlock: "Desbloquear",
    lockAll: "Bloquear tudo",
    unlockAll: "Desbloquear tudo"
  },
  statusPublished: "Publicado",
  sidebarLock: "Manter a barra lateral aberta",
  selectAllElementsInFrame: "Selecionar todos os elementos na moldura",
  removeAllElementsFromFrame: "Remover todos os elementos na moldura",
  eyeDropper: "Escolher cor da \xE1rea de desenho",
  textToDiagram: "Texto para diagrama",
  prompt: "Entrada de c\xF3digo",
  followUs: "Siga-nos",
  discordChat: "Conversa\xE7\xE3o no Discord",
  zoomToFitViewport: "Aproximar para enquadrar no cisualizador",
  zoomToFitSelection: "Aproximar para enquadrar a sele\xE7\xE3o",
  zoomToFit: "Aproximar para enquadrar todos os elementos",
  installPWA: "Instalar Excalidraw localmente (PWA)",
  autoResize: "Ativar redimensionamento autom\xE1tico do texto",
  imageCropping: "Corte de imagem",
  unCroppedDimension: "Dimens\xE3o sem corte",
  copyElementLink: "Copiar hiperliga\xE7\xE3o para o objeto",
  linkToElement: "Hiperliga\xE7\xE3o para o objeto",
  wrapSelectionInFrame: "Ajustar sele\xE7\xE3o no quadro",
  tab: "",
  shapeSwitch: ""
};
var elementLink = {
  title: "Hiperliga\xE7\xE3o para o objeto",
  desc: "Clique numa forma na tela ou cole uma hiperliga\xE7\xE3o.",
  notFound: "O objeto hiperligado n\xE3o foi encontrado na tela."
};
var library = {
  noItems: "Ainda n\xE3o foram adicionados itens...",
  hint_emptyLibrary: "Selecione um item na \xE1rea de desenho para adicion\xE1-lo aqui ou ent\xE3o instale uma biblioteca do reposit\xF3rio p\xFAblico abaixo.",
  hint_emptyPrivateLibrary: "Selecione um item na \xE1rea de desenho para adicion\xE1-lo aqui.",
  search: {
    inputPlaceholder: "",
    heading: "",
    noResults: "",
    clearSearch: ""
  }
};
var search = {
  title: "Pesquisar na \xE1rea de desenho",
  noMatch: "N\xE3o foram encontrados resultados...",
  singleResult: "resultado",
  multipleResults: "resultados",
  placeholder: "Pesquisar texto na \xE1rea de desenho...",
  frames: "",
  texts: ""
};
var buttons = {
  clearReset: "Limpar a \xE1rea de desenho",
  exportJSON: "Exportar para ficheiro",
  exportImage: "Exportar imagem...",
  export: "Guardar para...",
  copyToClipboard: "Copiar para a \xE1rea de transfer\xEAncia",
  copyLink: "Copiar hiperliga\xE7\xE3o",
  save: "Guardar no ficheiro atual",
  saveAs: "Guardar como",
  load: "Abrir",
  getShareableLink: "Obter uma hiperliga\xE7\xE3o de partilha",
  close: "Fechar",
  selectLanguage: "Selecionar idioma",
  scrollBackToContent: "Voltar ao conte\xFAdo",
  zoomIn: "Aproximar",
  zoomOut: "Afastar",
  resetZoom: "Repor amplia\xE7\xE3o",
  menu: "Menu",
  done: "Conclu\xEDdo",
  edit: "Editar",
  undo: "Desfazer",
  redo: "Refazer",
  resetLibrary: "Repor a biblioteca",
  createNewRoom: "Criar nova sala",
  fullScreen: "Ecr\xE3 inteiro",
  darkMode: "Modo escuro",
  lightMode: "Modo claro",
  systemMode: "Modo do sistema",
  zenMode: "Modo zen",
  objectsSnapMode: "Atrair aos objetos",
  exitZenMode: "Sair do modo zen",
  cancel: "Cancelar",
  saveLibNames: "",
  clear: "Limpar",
  remove: "Remover",
  embed: "Alternar incorpora\xE7\xE3o",
  publishLibrary: "",
  submit: "Enviar",
  confirm: "Confirmar",
  embeddableInteractionButton: "Clique para interagir"
};
var alerts = {
  clearReset: "Isto ir\xE1 limpar toda a \xE1rea de desenho. Tem a certeza?",
  couldNotCreateShareableLink: "N\xE3o foi poss\xEDvel criar uma hiperliga\xE7\xE3o partilh\xE1vel.",
  couldNotCreateShareableLinkTooBig: "N\xE3o foi poss\xEDvel criar uma hiperliga\xE7\xE3o partilh\xE1vel: a cena \xE9 muito grande",
  couldNotLoadInvalidFile: "N\xE3o foi poss\xEDvel carregar o ficheiro inv\xE1lido",
  importBackendFailed: "A importa\xE7\xE3o do servidor falhou.",
  cannotExportEmptyCanvas: "N\xE3o \xE9 poss\xEDvel exportar uma \xE1rea de desenho vazia.",
  couldNotCopyToClipboard: "N\xE3o foi poss\xEDvel copiar para a \xE1rea de transfer\xEAncia.",
  decryptFailed: "N\xE3o foi poss\xEDvel desencriptar os dados.",
  uploadedSecurly: "O envio foi protegido com encripta\xE7\xE3o de ponta a ponta, o que significa que o servidor do Excalidraw e terceiros n\xE3o podem ler o conte\xFAdo.",
  loadSceneOverridePrompt: "Se carregar um desenho externo substituir\xE1 o conte\xFAdo existente. Quer continuar?",
  collabStopOverridePrompt: "Ao interromper a sess\xE3o ir\xE1 substituir o \xFAltimo desenho guardado. Tem a certeza?\n\n(Caso queira manter o \xFAltimo desenho, simplesmente feche a janela do navegador.)",
  errorAddingToLibrary: "N\xE3o foi poss\xEDvel adicionar o item \xE0 biblioteca",
  errorRemovingFromLibrary: "N\xE3o foi poss\xEDvel remover o item da biblioteca",
  confirmAddLibrary: "Isto ir\xE1 adicionar {{numShapes}} forma(s) \xE0 sua biblioteca. Tem a certeza?",
  imageDoesNotContainScene: "Esta imagem parece n\xE3o conter dados de cenas. Ativou a incorpora\xE7\xE3o da cena durante a exporta\xE7\xE3o?",
  cannotRestoreFromImage: "N\xE3o foi poss\xEDvel restaurar a cena deste ficheiro de imagem",
  invalidSceneUrl: "N\xE3o foi poss\xEDvel importar a cena a partir do URL fornecido. Ou est\xE1 mal formado ou n\xE3o cont\xE9m dados JSON do Excalidraw v\xE1lidos.",
  resetLibrary: "Isto ir\xE1 limpar a sua biblioteca. Tem a certeza?",
  removeItemsFromsLibrary: "Apagar {{count}} item(ns) da biblioteca?",
  invalidEncryptionKey: "A chave de encripta\xE7\xE3o deve ter 22 caracteres. A colabora\xE7\xE3o ao vivo est\xE1 desativada.",
  collabOfflineWarning: "A liga\xE7\xE3o \xE0 Internet n\xE3o est\xE1 dispon\xEDvel.\nAs suas altera\xE7\xF5es n\xE3o ser\xE3o guardadas!",
  localStorageQuotaExceeded: ""
};
var errors = {
  unsupportedFileType: "Tipo de ficheiro n\xE3o suportado.",
  imageInsertError: "N\xE3o foi poss\xEDvel inserir a imagem, tente novamente mais tarde...",
  fileTooBig: "O ficheiro \xE9 muito grande. O tamanho m\xE1ximo permitido \xE9 {{maxSize}}.",
  svgImageInsertError: "N\xE3o foi poss\xEDvel inserir a imagem SVG. A marca\xE7\xE3o SVG parece inv\xE1lida.",
  failedToFetchImage: "Falha ao processar a imagem.",
  cannotResolveCollabServer: "N\xE3o foi poss\xEDvel fazer a liga\xE7\xE3o ao servidor colaborativo. Por favor volte a carregar a p\xE1gina e tente novamente.",
  importLibraryError: "N\xE3o foi poss\xEDvel carregar a biblioteca",
  saveLibraryError: "N\xE3o foi poss\xEDvel guardar a biblioteca no armazenamento. Guarde a sua biblioteca num ficheiro local para garantir que n\xE3o perde as altera\xE7\xF5es.",
  collabSaveFailed: "N\xE3o foi poss\xEDvel guardar na base de dados de backend. Se os problemas persistirem, guarde o ficheiro localmente para garantir que n\xE3o perde o seu trabalho.",
  collabSaveFailed_sizeExceeded: "N\xE3o foi poss\xEDvel guardar na base de dados de backend, parece que a a \xE1rea de desenho \xE9 muito grande. Deve guardar o ficheiro localmente para garantir que n\xE3o perde o seu trabalho.",
  imageToolNotSupported: "As imagens est\xE3o desativadas.",
  brave_measure_text_error: {
    line1: "Parece que est\xE1 a utilizar o navegador Brave com a configura\xE7\xE3o <bold>Bloquear impress\xF5es digitais (fingerprinting) no modo restrito/agressivo</bold> ativada.",
    line2: "Isto pode resultar na quebra dos <bold>Elementos de texto</bold> nos seus desenhos.",
    line3: "Recomendamos vivamente a desativa\xE7\xE3o desta configura\xE7\xE3o. Pode seguir <link>estes passos</link> para saber como o fazer.",
    line4: "Se a desativa\xE7\xE3o desta configura\xE7\xE3o n\xE3o corrigir a apresenta\xE7\xE3o de elementos de texto, reporte um <issueLink>problema</issueLink> no nosso GitHub ou diga-nos no <discordLink>Discord</discordLink>"
  },
  libraryElementTypeError: {
    embeddable: "Os elementos incorpor\xE1veis n\xE3o podem ser adicionados \xE0 biblioteca.",
    iframe: "Os elementos IFrame n\xE3o podem ser adicionados \xE0 biblioteca.",
    image: "Brevemente ser\xE1 poss\xEDvel adicionar imagens \xE0 biblioteca!"
  },
  asyncPasteFailedOnRead: "N\xE3o foi poss\xEDvel colar (n\xE3o foi poss\xEDvel ler da \xE1rea de transfer\xEAncia do sistema).",
  asyncPasteFailedOnParse: "N\xE3o foi poss\xEDvel colar.",
  copyToSystemClipboardFailed: "N\xE3o foi poss\xEDvel copiar para a \xE1rea de transfer\xEAncia."
};
var toolBar = {
  selection: "Sele\xE7\xE3o",
  lasso: "",
  image: "Inserir imagem",
  rectangle: "Ret\xE2ngulo",
  diamond: "Losango",
  ellipse: "Elipse",
  arrow: "Seta",
  line: "Linha",
  freedraw: "Desenhar",
  text: "Texto",
  library: "Biblioteca",
  lock: "Manter a ferramenta selecionada ativa ap\xF3s desenhar",
  penMode: "Modo caneta - impedir toque",
  link: "Adicionar / atualizar hiperliga\xE7\xE3o para uma forma selecionada",
  eraser: "Borracha",
  frame: "Ferramenta de moldura",
  magicframe: "Esquema para c\xF3digo",
  embeddable: "Incorporar web",
  laser: "Ponteiro laser",
  hand: "M\xE3o (ferramenta de deslocar vista)",
  extraTools: "Mais ferramentas",
  mermaidToExcalidraw: "Mermaid para Excalidraw",
  convertElementType: ""
};
var element = {
  rectangle: "Ret\xE2ngulo",
  diamond: "Diamante",
  ellipse: "Ellipse",
  arrow: "Seta",
  line: "Linha",
  freedraw: "Desenho livre",
  text: "Texto",
  image: "Imagem",
  group: "Grupo",
  frame: "Frame",
  magicframe: "Wireframe para c\xF3digo",
  embeddable: "Web Embed",
  selection: "Sele\xE7\xE3o",
  iframe: "IFrame"
};
var headings = {
  canvasActions: "A\xE7\xF5es da \xE1rea de desenho",
  selectedShapeActions: "A\xE7\xF5es das formas selecionadas",
  shapes: "Formas"
};
var hints = {
  dismissSearch: "",
  canvasPanning: "",
  linearElement: "Clique para iniciar v\xE1rios pontos, arraste para uma \xFAnica linha",
  arrowTool: "",
  arrowBindModifiers: "",
  freeDraw: "Clique e arraste, largue quando terminar",
  text: "Dica: tamb\xE9m pode adicionar texto clicando duas vezes em qualquer lugar com a ferramenta de sele\xE7\xE3o",
  embeddable: "Clique e arraste para criar uma incorpora\xE7\xE3o do site",
  text_selected: "",
  text_editing: "",
  linearElementMulti: "",
  lockAngle: "",
  resize: "",
  resizeImage: "",
  rotate: "",
  lineEditor_info: "",
  lineEditor_line_info: "",
  lineEditor_pointSelected: "",
  lineEditor_nothingSelected: "",
  publishLibrary: "Publique a sua pr\xF3pria biblioteca",
  bindTextToElement: "",
  createFlowchart: "",
  deepBoxSelect: "",
  eraserRevert: "",
  firefox_clipboard_write: 'Esta fun\xE7\xE3o pode provavelmente ser ativada definindo a op\xE7\xE3o "dom.events.asyncClipboard.clipboardItem" como "true". Para alterar os sinalizadores do navegador no Firefox, visite a p\xE1gina "about:config".',
  disableSnapping: "",
  enterCropEditor: "",
  leaveCropEditor: ""
};
var canvasError = {
  cannotShowPreview: "N\xE3o \xE9 poss\xEDvel mostrar uma pr\xE9-visualiza\xE7\xE3o",
  canvasTooBig: "A \xE1rea de desenho pode ser muito grande.",
  canvasTooBigTip: "Dica: tente aproximar um pouco os elementos mais distantes."
};
var errorSplash = {
  headingMain: "Foi encontrado um erro. Tente <button>recarregar a p\xE1gina</button>.",
  clearCanvasMessage: "Se o recarregar da p\xE1gina n\xE3o funcionar, tente <button>limpar a \xE1rea de desenho</button>.",
  clearCanvasCaveat: " Isto resultar\xE1 na perda do trabalho ",
  trackedToSentry: "O erro com o identificador {{eventId}} foi rastreado no nosso sistema.",
  openIssueMessage: "Tivemos muito cuidado para n\xE3o incluir as informa\xE7\xF5es da sua cena no erro. Se a sua cena n\xE3o for privada, considere fazer o acompanhamento no nosso <button>rastreador de problemas</button>. Inclua as informa\xE7\xF5es abaixo copiando e colando no problema do GitHub.",
  sceneContent: "Conte\xFAdo da cena:"
};
var shareDialog = {
  or: "Ou"
};
var roomDialog = {
  desc_intro: "Convide pessoas para colaborar no seu desenho.",
  desc_privacy: "N\xE3o se preocupe, a sess\xE3o \xE9 encriptada e totalmente privada. Nem mesmo o nosso servidor pode ver o que est\xE1 a desenhar.",
  button_startSession: "Iniciar sess\xE3o",
  button_stopSession: "Parar sess\xE3o",
  desc_inProgressIntro: "A sess\xE3o de colabora\xE7\xE3o ao vivo est\xE1 agora a decorrer.",
  desc_shareLink: "Partilhe esta hiperliga\xE7\xE3o com qualquer pessoa com quem queira colaborar:",
  desc_exitSession: "Se interromper a sess\xE3o, ser\xE1 desconectado da sala, mas poder\xE1 continuar a trabalhar com a cena localmente. Tenha em aten\xE7\xE3o que isto n\xE3o afetar\xE1 as outras pessoas, que poder\xE3o continuar a colaborar na sua vers\xE3o.",
  shareTitle: "Participe numa sess\xE3o de colabora\xE7\xE3o ao vivo no Excalidraw"
};
var errorDialog = {
  title: "Erro"
};
var exportDialog = {
  disk_title: "Guardar no disco",
  disk_details: "Exportar os dados da cena para um ficheiro do qual poder\xE1 importar mais tarde.",
  disk_button: "Guardar num ficheiro",
  link_title: "Hiperliga\xE7\xE3o partilh\xE1vel",
  link_details: "Exportar como uma hiperliga\xE7\xE3o de apenas leitura.",
  link_button: "Exportar para hiperliga\xE7\xE3o",
  excalidrawplus_description: "Guardar a cena no seu espa\xE7o de trabalho do Excalidraw+.",
  excalidrawplus_button: "Exportar",
  excalidrawplus_exportError: "N\xE3o foi poss\xEDvel exportar para o Excalidraw+ neste momento..."
};
var helpDialog = {
  blog: "Leia o nosso blogue",
  click: "clicar",
  deepSelect: "Sele\xE7\xE3o profunda",
  deepBoxSelect: "Sele\xE7\xE3o profunda dentro da caixa, impedindo que seja arrastada",
  createFlowchart: "Criar um fluxograma a partir de um elemento gen\xE9rico",
  navigateFlowchart: "Navegar num fluxograma",
  curvedArrow: "Seta curva",
  curvedLine: "Linha curva",
  documentation: "Documenta\xE7\xE3o",
  doubleClick: "clique duplo",
  drag: "arrastar",
  editor: "Editor",
  editLineArrowPoints: "Editar pontos da linha / seta",
  editText: "Editar texto / adicionar etiqueta",
  github: "Encontrou algum problema? Informe-nos",
  howto: "Siga os nossos guias",
  or: "ou",
  preventBinding: "Prevenir fixa\xE7\xE3o de seta",
  tools: "Ferramentas",
  shortcuts: "Atalhos de teclado",
  textFinish: "Finalizar edi\xE7\xE3o (editor de texto)",
  textNewLine: "Adicionar nova linha (editor de texto)",
  title: "Ajuda",
  view: "Visualiza\xE7\xE3o",
  zoomToFit: "Enquadrar vista em todos os elementos",
  zoomToSelection: "Enquadrar vista na sele\xE7\xE3o",
  toggleElementLock: "Bloquear / desbloquear sele\xE7\xE3o",
  movePageUpDown: "Mover p\xE1gina para cima / baixo",
  movePageLeftRight: "Mover p\xE1gina para esquerda / direita",
  cropStart: "Recortar imagem",
  cropFinish: "Finalizar recorte da imagem"
};
var clearCanvasDialog = {
  title: "Apagar \xE1rea de desenho"
};
var publishDialog = {
  title: "Publicar biblioteca",
  itemName: "Nome do item",
  authorName: "Nome do autor",
  githubUsername: "Nome de utilizador no GitHub",
  twitterUsername: "Nome de utilizador no Twitter",
  libraryName: "Nome da biblioteca",
  libraryDesc: "Descri\xE7\xE3o da biblioteca",
  website: "P\xE1gina web",
  placeholder: {
    authorName: "Introduza o seu nome ou nome de utilizador",
    libraryName: "Nome da sua biblioteca",
    libraryDesc: "Descri\xE7\xE3o da sua biblioteca para ajudar as pessoas a compreenderem a sua utiliza\xE7\xE3o",
    githubHandle: "Identificador do GitHub (handle opcional), para que possa editar a biblioteca depois desta ser enviada para revis\xE3o",
    twitterHandle: "Nome de utilizador no Twitter (opcional), para que saibamos a quem atribuir os cr\xE9ditos quando promovermos atrav\xE9s do Twitter",
    website: "Hiperliga\xE7\xE3o para o seu s\xEDtio web pessoal ou outro (opcional)"
  },
  errors: {
    required: "Obrigat\xF3rio",
    website: "Introduza um URL v\xE1lido"
  },
  noteDescription: "Envie a sua biblioteca para ser inclu\xEDda no <link>reposit\xF3rio de bibliotecas p\xFAblicas</link> para que outras pessoas a possam utilizar nos seus desenhos.",
  noteGuidelines: "A biblioteca precisa de ser aprovada manualmente primeiro. Por favor, leia as <link>orienta\xE7\xF5es</link> antes de enviar. Precisar\xE1 de uma conta GitHub para comunicar e fazer altera\xE7\xF5es, se solicitado, mas n\xE3o \xE9 estritamente necess\xE1rio.",
  noteLicense: "Ao submeter, concorda que a biblioteca ser\xE1 publicada ao abrigo da <link>Licen\xE7a MIT</link>, o que, resumidamente, significa que qualquer pessoa pode utiliz\xE1-la sem restri\xE7\xF5es.",
  noteItems: "Cada item de biblioteca deve ter o seu pr\xF3prio nome para que possa ser filtrado. Ser\xE3o inclu\xEDdos os seguintes itens de biblioteca:",
  atleastOneLibItem: "Por favor selecione pelo menos um item da biblioteca para come\xE7ar",
  republishWarning: "Nota: alguns dos itens selecionados est\xE3o marcados como j\xE1 publicados / submetidos. S\xF3 deve voltar a submeter itens quando atualizar uma biblioteca ou submiss\xE3o existente."
};
var publishSuccessDialog = {
  title: "Biblioteca enviada",
  content: "Obrigado {{authorName}}. A sua biblioteca foi enviada para an\xE1lise. Pode acompanhar o estado dela <link>aqui</link>"
};
var confirmDialog = {
  resetLibrary: "Repor a biblioteca",
  removeItemsFromLib: "Remover os itens selecionados da biblioteca"
};
var imageExportDialog = {
  header: "Exportar imagem",
  label: {
    withBackground: "Fundo",
    onlySelected: "Apenas o selecionado",
    darkMode: "Modo escuro",
    embedScene: "Cena incorporada",
    scale: "Redimensionar",
    padding: "Espa\xE7amento"
  },
  tooltip: {
    embedScene: "Os dados da cena ser\xE3o guardados no ficheiro PNG / SVG exportado para que a cena possa ser restaurada a partir dele.\nIsto aumentar\xE1 o tamanho do ficheiro exportado."
  },
  title: {
    exportToPng: "Exportar em PNG",
    exportToSvg: "Exportar em SVG",
    copyPngToClipboard: "Copiar PNG para \xE1rea de transfer\xEAncia"
  },
  button: {
    exportToPng: "PNG",
    exportToSvg: "SVG",
    copyPngToClipboard: "Copiar para a \xE1rea de transfer\xEAncia"
  }
};
var encrypted = {
  tooltip: "Os seus desenhos s\xE3o encriptados de ponta a ponta para que os servidores do Excalidraw nunca os vejam.",
  link: "Publica\xE7\xE3o no blogue sobre a encripta\xE7\xE3o de ponta a ponta no Excalidraw"
};
var stats = {
  angle: "\xC2ngulo",
  shapes: "Formas",
  height: "Altura",
  scene: "Cena",
  selected: "Selecionado",
  storage: "Armazenamento",
  fullTitle: "\xC1rea de desenho e propriedades da forma",
  title: "Propriedades",
  generalStats: "Geral",
  elementProperties: "Propriedades da forma",
  total: "Total",
  version: "Vers\xE3o",
  versionCopy: "Clique para copiar",
  versionNotAvailable: "Vers\xE3o n\xE3o dispon\xEDvel",
  width: "Largura"
};
var toast = {
  addedToLibrary: "Adicionado \xE0 biblioteca",
  copyStyles: "Estilos copiados.",
  copyToClipboard: "Copiado para a \xE1rea de transfer\xEAncia.",
  copyToClipboardAsPng: "{{exportSelection}} copiado para a \xE1rea de transfer\xEAncia como PNG\n({{exportColorScheme}})",
  copyToClipboardAsSvg: "{{exportSelection}} copiado para a \xE1rea de transfer\xEAncia como SVG\n({{exportColorScheme}})",
  fileSaved: "Ficheiro guardado.",
  fileSavedToFilename: "Guardado como {filename}",
  canvas: "\xE1rea de desenho",
  selection: "sele\xE7\xE3o",
  pasteAsSingleElement: "Usar {{shortcut}} para colar como um \xFAnico elemento,\nou colar num editor de texto existente",
  unableToEmbed: "A incorpora\xE7\xE3o deste URL n\xE3o \xE9 atualmente permitida. Reporte o problema no GitHub para solicitar a inclus\xE3o do URL na lista branca para deixar de o bloquear",
  unrecognizedLinkFormat: "A liga\xE7\xE3o que incorporou n\xE3o corresponde ao formato esperado. Tente colar a cadeia de caracteres 'incorporada' fornecida pelo s\xEDtio de origem",
  elementLinkCopied: "Hiperliga\xE7\xE3o copiada para a \xE1rea de transfer\xEAncia"
};
var colors = {
  transparent: "Transparente",
  black: "Preto",
  white: "Branco",
  red: "Vermelho",
  pink: "Rosa",
  grape: "Uva",
  violet: "Violeta",
  gray: "Cinza",
  blue: "Azul",
  cyan: "Ciano",
  teal: "Verde-azulado",
  green: "Verde",
  yellow: "Amarelo",
  orange: "Laranja",
  bronze: "Bronze"
};
var welcomeScreen = {
  app: {
    center_heading: "",
    center_heading_line2: "",
    center_heading_line3: "",
    center_heading_plus: "Queria antes ir para o Excalidraw+?",
    menuHint: "Exportar, prefer\xEAncias, idiomas..."
  },
  defaults: {
    menuHint: "Exportar, prefer\xEAncias e outros...",
    center_heading: "Diagramas. Feito. Simples.",
    toolbarHint: "Escolha uma ferramenta e comece a desenhar!",
    helpHint: "Atalhos e ajuda"
  }
};
var colorPicker = {
  color: "",
  mostUsedCustomColors: "Cores personalizadas mais usadas",
  colors: "Cores",
  shades: "Tons",
  hexCode: "C\xF3digo hex",
  noShades: "Sem tons dispon\xEDveis para esta cor"
};
var overwriteConfirm = {
  action: {
    exportToImage: {
      title: "Exportar como imagem",
      button: "Exportar como imagem",
      description: "Exportar os dados da cena como uma imagem a partir da qual pode importar mais tarde."
    },
    saveToDisk: {
      title: "Guardar no disco",
      button: "Guardar no disco",
      description: "Exportar os dados da cena para um ficheiro do qual poder\xE1 importar mais tarde."
    },
    excalidrawPlus: {
      title: "Excalidraw+",
      button: "Exportar para Excalidraw+",
      description: "Guardar a cena no seu espa\xE7o de trabalho do Excalidraw+."
    }
  },
  modal: {
    loadFromFile: {
      title: "Carregar a partir de ficheiro",
      button: "Carregar a partir de ficheiro",
      description: "Carregar a partir de um ficheiro ir\xE1 <bold>substituir o conte\xFAdo existente</bold>.<br></br>Pode fazer primeiro uma c\xF3pia de seguran\xE7a do seu desenho utilizando uma das op\xE7\xF5es abaixo."
    },
    shareableLink: {
      title: "Carregar da hiperliga\xE7\xE3o",
      button: "Substituir o meu conte\xFAdo",
      description: "Carregar um desenho externo ir\xE1 <bold>substituir o conte\xFAdo existente</bold>.<br></br>Pode fazer primeiro uma c\xF3pia de seguran\xE7a do seu desenho utilizando uma das op\xE7\xF5es abaixo."
    }
  }
};
var mermaid = {
  title: "Mermaid para Excalidraw",
  button: "Inserir",
  description: "Atualmente apenas s\xE3o suportados diagramas <flowchartLink>fluxo</flowchartLink>, <sequenceLink>sequ\xEAncia, </sequenceLink> e <classLink>classe</classLink>. Os outros tipos ser\xE3o renderizados como imagem no Excalidraw.",
  syntax: "Sintaxe Mermaid",
  preview: "Pr\xE9-visualizar",
  label: "",
  inputPlaceholder: ""
};
var ttd = {
  error: ""
};
var chat = {
  inputPlaceholder: "",
  inputPlaceholderWithMessages: "",
  generating: "",
  rateLimitRemaining: "",
  role: {
    user: "",
    assistant: "",
    system: ""
  },
  aiBeta: "",
  label: "",
  menu: "",
  newChat: "",
  deleteChat: "",
  deleteMessage: "",
  viewAsMermaid: "",
  placeholder: {
    title: "",
    description: "",
    hint: ""
  },
  preview: "",
  insert: "",
  retry: "",
  errors: {
    promptTooShort: "",
    promptTooLong: "",
    generationFailed: "",
    invalidDiagram: "",
    fixInMermaid: "",
    aiRepair: "",
    requestAborted: "",
    requestFailed: "",
    mermaidParseError: ""
  },
  rateLimit: {
    messageLimit: "",
    generalRateLimit: "",
    messageLimitInputPlaceholder: ""
  },
  upsellBtnLabel: ""
};
var quickSearch = {
  placeholder: "Pesquisa r\xE1pida"
};
var fontList = {
  badge: {
    old: "antigo"
  },
  sceneFonts: "Nesta cena",
  availableFonts: "Fontes dispon\xEDveis",
  empty: "Nenhuma fonte encontrada"
};
var userList = {
  empty: "Nenhum utilizador encontrado",
  hint: {
    text: "Clique no utilizador para o seguir",
    followStatus: "Est\xE1 a seguir este utilizador",
    inCall: "Utilizador est\xE1 numa chamada de voz",
    micMuted: "O microfone do utilizador est\xE1 desligado",
    isSpeaking: "O utilizador est\xE1 a falar"
  }
};
var commandPalette = {
  title: "Paleta de comandos",
  shortcuts: {
    select: "Selecionar",
    confirm: "Confirmar",
    close: "Fechar"
  },
  recents: "Usado recentemente",
  search: {
    placeholder: "Procure menus, comandos e descubra pedras preciosas ocultas",
    noMatch: "Nenhum comando correspondente..."
  },
  itemNotAvailable: "Comando n\xE3o est\xE1 dispon\xEDvel...",
  shortcutHint: "Para a paleta de comandos, use {{shortcut}}"
};
var keys = {
  ctrl: "",
  option: "",
  cmd: "",
  alt: "",
  escape: "",
  enter: "",
  shift: "",
  spacebar: "",
  delete: "",
  mmb: ""
};
var pt_PT_default = {
  labels,
  elementLink,
  library,
  search,
  buttons,
  alerts,
  errors,
  toolBar,
  element,
  headings,
  hints,
  canvasError,
  errorSplash,
  shareDialog,
  roomDialog,
  errorDialog,
  exportDialog,
  helpDialog,
  clearCanvasDialog,
  publishDialog,
  publishSuccessDialog,
  confirmDialog,
  imageExportDialog,
  encrypted,
  stats,
  toast,
  colors,
  welcomeScreen,
  colorPicker,
  overwriteConfirm,
  mermaid,
  ttd,
  chat,
  quickSearch,
  fontList,
  userList,
  commandPalette,
  keys
};
export {
  alerts,
  buttons,
  canvasError,
  chat,
  clearCanvasDialog,
  colorPicker,
  colors,
  commandPalette,
  confirmDialog,
  pt_PT_default as default,
  element,
  elementLink,
  encrypted,
  errorDialog,
  errorSplash,
  errors,
  exportDialog,
  fontList,
  headings,
  helpDialog,
  hints,
  imageExportDialog,
  keys,
  labels,
  library,
  mermaid,
  overwriteConfirm,
  publishDialog,
  publishSuccessDialog,
  quickSearch,
  roomDialog,
  search,
  shareDialog,
  stats,
  toast,
  toolBar,
  ttd,
  userList,
  welcomeScreen
};
//# sourceMappingURL=pt-PT-JZOCCWYW.js.map
